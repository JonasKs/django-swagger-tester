...
def fetch_generated_schema(url: str, method: str, status_code: Union[str, int]) -> dict:
    """
    ...
    try:
        ...
        raise SpecificationError(f'No path found for url `{url}`. Valid urls include {", ".join([key for key in schema.keys()])}')
    try:
